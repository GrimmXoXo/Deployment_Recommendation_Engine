...
class UserProfiling:
    ...
    def cosine_similarity(x, y):
        ...
        dot_product = np.dot(x, y.T)  # Transpose y to align dimensions for dot product
        norm_x = np.linalg.norm(x)
        norm_y = np.linalg.norm(y, axis=1)  # Compute norm along axis=1 (rows) for y
    ...
    def recommend_books( user_id=None, book_id=None, top_n=10):
        ...
        if user_id is not None:
            ...
            similarities = UserProfiling.cosine_similarity(user_profile, item_profiles)
        elif book_id is not None:
            if book_id not in item_profiles.index:
                return [], []
            item_profile = item_profiles.loc[book_id].values.reshape(1, -1)
            similarities = UserProfiling.cosine_similarity(item_profile, item_profiles)
        else:
            return [], []
